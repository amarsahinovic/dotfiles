...
- [[#important-to-load-first][Important to load first]]
  ...
  - [[#traditional-emacs-style-keybindings][Traditional Emacs-style Keybindings]]
...
** Traditional Emacs-style Keybindings
Emacs follows conventions for organizing keybindings under standard prefixes.
This section provides traditional Emacs-style bindings alongside the SPC leader key bindings above.

Standard Emacs prefixes:
- C-x: File, buffer, window operations (built-in)
- C-c: User and mode-specific commands (reserved for users)
- C-h: Help system (built-in)
- M-s: Search operations (built-in)
- C-x p: Project commands (built-in, enhanced with consult-project-extra)

Custom prefixes defined below:
- C-c f: Find operations (files, projects, recent files)
- C-c s: Search operations (grep, line search, symbols)
- C-c b: Buffer operations (switch, kill, reload)
- C-c g: Git/Version control (magit commands)
- C-c e: Language/Eglot operations (LSP, diagnostics)
- C-c t: Toggle operations (line numbers, themes, modes)
- C-c p: Project operations (projectile-specific commands)
